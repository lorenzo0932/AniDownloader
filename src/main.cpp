...
static void cliSignalHandler(int) {
    if (g_stopPtr)
        g_stopPtr->store(true, std::memory_order_relaxed);
    if (g_webRunningPtr)
        g_webRunningPtr->store(false, std::memory_order_relaxed);
...
void refreshTerminal(bool burst, bool force = false) {
    ...
    if (!force && now - lastDraw < std::chrono::milliseconds(200))
        return;
    lastDraw = now;
    ...
    for (auto const& [name, status] : g_statusMap) {
        if (status.find("Analisi:") != std::string::npos) {
            hasAnalisi = true;
            break;
        }
    }
    ...
    std::cout << "   " << spin << " AniDownloader C++ | Mod: " << (burst ? "BURST 🚀" : "SILENT ☁️")
              << " | T: " << elapsed << "s\n";
    std::cout << "==========================================================\n";
    ...
    if (g_statusMap.empty()) {
        ...
        for (auto const& [name, status] : g_statusMap) {
            ...
            std::cout << " - " << std::left << std::setw(35) << disp << " : " << status
                      << "\033[K\n";
        }
    }
    std::cout << "==========================================================\n" << std::flush;
...
int main(int argc, char* argv[]) {
    ...
    if (burstMode)
        refreshTerminal(burstMode, true);

    std::cout << "\n\n--- RESOCONTO FINALE ---\n";
    for (const auto& r : g_reports) {
        std::string label = (r.episodeNumber > 0)
                                ? r.name + " (Ep " + std::to_string(r.episodeNumber) + ")"
                                : r.name;
        if (!r.success) {
            std::cout << "❌ " << std::left << std::setw(35) << label << " | Errore: " << r.error
                      << "\n";
        } else {
            std::cout << "✅ " << std::left << std::setw(35) << label << " | DL: " << std::fixed
                      << std::setprecision(1) << r.dlTime << "s"
                      << " | Conv: " << r.convTime << "s\n";
        }
    }
