...
static std::string extractEpisodeId(const std::string& pageUrl) {
    ...
    if (pos == std::string::npos) return pageUrl;
    ...
    ScraperProgressCb progressCb)
{
    ...
    if (progressCb) progressCb("Analisi: " + std::to_string(candidates.size()) + " episodi da verificare...");

    cpr::Header apiHeaders = {
        {"User-Agent", ScraperUtils::platformUserAgent()},
    ...
    for (const auto& candidate : candidates) {
        ...
        checked++;
        if (progressCb) progressCb("Analisi: verifica episodio " + std::to_string(checked) + "/" +
        ...
        std::string grabber;
        try {
            cpr::Response r = ScraperUtils::httpGetWithRetry(cpr::Url{apiUrl}, apiHeaders, 3, 2000);
