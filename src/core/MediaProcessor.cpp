...
bool MediaProcessor::convertAndVerify(const std::string& inputPath, const std::string& name, int chunks, double& outTime) {
    ...
    for (const auto& p : parts) {
        ...
        std::string cmd = "ffmpeg -v error -y -i " + Q(p.string()) + " -c:v libx265 -crf 23 -x265-params hist-scenecut=1 -preset veryfast -threads 8  -progress " + Q(progP.string()) + " " + Q(outP.string()) + " > /dev/null 2>&1";
