...
namespace Core {
    namespace MediaProbe {
        ...
        double getVideoDuration(const std::string& filePath, std::atomic<bool>& stopSignal) {
            ...
            std::string output;
            ProcessUtils::runCommand(cmd, stopSignal,
        ...
        std::string getVideoCodec(const std::string& filePath, std::atomic<bool>& stopSignal) {
            ...
            int status = ProcessUtils::runCommand(cmd, stopSignal,
                                                  [&](const std::string& line) { output += line; });
            if (status != 0)
                output.clear();
        ...
        bool isMediaFileHealthy(const std::string& filePath, std::atomic<bool>& stopSignal) {
            ...
            std::string verifyCmd =
                "ffmpeg -v error -xerror -i " + ScraperUtils::Q(filePath) + " -c copy -f null -";
